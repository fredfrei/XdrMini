...
Window {
    ...
    // XDRTABLET_TMC_DISTANCE_FILTER_V1
    property bool distanceFilterEnabled: true

    function distanceFilterLocation() {
        let value = tmcLocationField.text.trim()
        value = value.replace(";", ",")
    ...
    function distanceFilterRadiusKm() {
        const value = Number(
            tmcRadiusField.text.trim().replace(",", "."))
    ...
    function distanceFilterValid() {
        return distanceFilterLocation() !== null
                && distanceFilterRadiusKm() > 0
    }
    ...
    function tmcBlockDistanceKm(block) {
        const origin = distanceFilterLocation()
        if (origin === null || !block || block.length === 0)
            return -1

        // Decoderzeilen enthalten Koordinaten hinter einem '|', z.B.
        // "Ort (LTN 1): A5 | 48.12345,7.98765".
        // Bei Strecken werden Ort, Start und Ende geprüft; es zählt der
        // nächstgelegene vorhandene LCL-Punkt.
    ...
    function tmcBlockInDistance(block) {
        if (!distanceFilterEnabled)
            return true

        // Bei fehlerhafter Eingabe nicht still alles ausblenden.
        if (!distanceFilterValid())
            return true

        const distance = tmcBlockDistanceKm(block)

        // Ohne geografische LCL-Koordinate ist eine Entfernung nicht
        // bestimmbar; bei aktivem Filter wird die Meldung ausgeblendet.
        return distance >= 0
                && distance <= distanceFilterRadiusKm()
    }

    function distanceFilteredTmcBlocks(fullText) {
        const blocks = tmcMessageBlocks(fullText)
        const result = []

        for (let i = 0; i < blocks.length; ++i) {
            if (tmcBlockInDistance(blocks[i]))
    ...
    function distanceFilterStatusText() {
        if (!distanceFilterEnabled)
            return "Entfernungsfilter ausgeschaltet"

        if (distanceFilterLocation() === null)
            return "Standort ungültig – Format: 48.00960666741811, 7.833772430212739"

        const radius = distanceFilterRadiusKm()
        if (radius <= 0)
            return "Umkreis ungültig – bitte Kilometer größer 0 eingeben"

        const inRange = distanceFilteredTmcBlocks(
            tmcWindow.client.tmcMessagesText).length

        return inRange + " von "
                + tmcWindow.client.tmcMessageCount
                + " Meldungen innerhalb " + radius + " km"
    }
    ...
    function compactTmcMessage(block) {
        ...
        for (let i = 0; i < lines.length; ++i) {
            ...
            if (i === 0) {
                ...
                const distance = tmcBlockDistanceKm(block)
                if (distance >= 0)
                    out.push("Entfernung: " + distance.toFixed(1) + " km")
    ...
    function selectedTmcBlocks(fullText) {
        const blocks = distanceFilteredTmcBlocks(fullText)
    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            GridLayout {
                ...
                Label {
                    Layout.fillWidth: true
                    text: tmcWindow.displayedTmcMessageCount(
            ...
            GridLayout {
                ...
                TextField {
                    ...
                    Layout.columnSpan: 3
                    Layout.fillWidth: true
                    text: "48.00960666741811, 7.833772430212739"
                    placeholderText: "Breitengrad, Längengrad"
                    selectByMouse: true
                    font.family: "monospace"
                }
                ...
                CheckBox {
                    id: tmcDistanceFilterCheck
                    text: "Filter aktiv"
                    checked: tmcWindow.distanceFilterEnabled
                    onToggled:
                        tmcWindow.distanceFilterEnabled = checked
                }

                Label {
                    Layout.columnSpan: 4
                    Layout.fillWidth: true
                    text: tmcWindow.distanceFilterStatusText()
                    color: tmcWindow.distanceFilterEnabled
                           && !tmcWindow.distanceFilterValid()
                           ? "#9a3d32"
                           : tmcWindow.mutedInk
            ...
            ScrollView {
                ...
                TextArea {
                    ...
                    text: {
                        ...
                        if (tmcWindow.distanceFilterEnabled
                                && tmcWindow.distanceFilterValid()
                                && tmcWindow.distanceFilteredTmcBlocks(fullText).length === 0) {
                            return "Keine aktuelle TMC-Meldung im eingestellten Umkreis."
                        }

                        return "Keine wichtigen Verkehrsmeldungen im Filter. "
                               + "Mit „Alle Meldungen“ werden alle Meldungen "
                               + "innerhalb des eingestellten Umkreises angezeigt."
                    }
            ...
            Label {
                ...
                text: tmcWindow.showTechnicalDetails
                      ...
                      : (tmcWindow.showAllMessages
                         ...
                         : "Gefilterte Ansicht: höchstens 15 Meldungen innerhalb des eingestellten Umkreises, "
                           + "wichtige Ereignisse zuerst. Reine Baustellenmeldungen ohne Sperrung oder "
                           + "Behinderung werden ausgeblendet.")
