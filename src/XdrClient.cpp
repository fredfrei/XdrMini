...
constexpr int ConnectionWatchdogIntervalMs = 1000;
constexpr qint64 ConnectionSilenceTimeoutMs = 6LL * 1000LL;
...
XdrClient::XdrClient(QObject *parent) : QObject(parent)
{
    ...
    connectionWatchdogTimer_.setInterval(ConnectionWatchdogIntervalMs);
    ...
    connect(&connectionWatchdogTimer_, &QTimer::timeout,
            this, &XdrClient::checkConnectionAlive);
...
int XdrClient::wam() const { return wam_; }
int XdrClient::usn() const { return usn_; }
...
{
    pendingDisconnectStatus_.clear();
    lastReceiveMs_ = 0;
...
{
    const QString disconnectStatus = pendingDisconnectStatus_;
    pendingDisconnectStatus_.clear();
    lastReceiveMs_ = 0;
    ...
    if (!disconnectStatus.isEmpty())
        setStatusText(disconnectStatus);
    else if (socket_.error() == QAbstractSocket::RemoteHostClosedError && !wasReady)
...
void XdrClient::onSerialError(QSerialPort::SerialPortError error)
{
    ...
    if (error == QSerialPort::ResourceError ||
        ...
        closeLostUsbConnection(
            QStringLiteral("USB-Verbindung unterbrochen: %1")
                .arg(serial_.errorString()));
    }
}

void XdrClient::markConnectionActivity()
{
    lastReceiveMs_ = QDateTime::currentMSecsSinceEpoch();
}

void XdrClient::closeLostUsbConnection(const QString &message)
{
    if (serial_.isOpen())
        serial_.close();
...
void XdrClient::checkConnectionAlive()
{
    if (!connected()) {
        lastReceiveMs_ = 0;
        return;
    ...
    if (usbMode_) {
        ...
        const QString openPort = serial_.portName();

        for (const QSerialPortInfo &info :
             QSerialPortInfo::availablePorts()) {
            if (info.portName() == openPort ||
                info.systemLocation() == openPort) {
                portPresent = true;
                break;
            }
        }
        ...
        if (!portPresent) {
            closeLostUsbConnection(
                QStringLiteral("USB-Kabel wurde getrennt"));
            return;
        }
    }
    ...
    if (now - lastReceiveMs_ <= ConnectionSilenceTimeoutMs)
        return;

    if (usbMode_) {
        closeLostUsbConnection(
            QStringLiteral("USB-Verbindung unterbrochen – keine Daten vom Radio"));
        return;
    }

    pendingDisconnectStatus_ =
        QStringLiteral("WLAN/TCP-Verbindung unterbrochen – keine Daten vom Radio");
    socket_.abort();
}
...
void XdrClient::processLine(const QString &line)
{
    ...
    if (line.startsWith(QStringLiteral("Ss")) ||
        ...
        // PE5PVB Ss/Sm: Feld 2 = WAM, Feld 3 = USN.
        // Kleine Werte bedeuten hier weniger Störungen/Rauschen.
...
void XdrClient::updatePsSegment(int segment, quint16 blockD)
{
    ...
    psBuffer_[offset] =
        decodeRdsCharacter(static_cast<quint8>(blockD >> 8));
    psBuffer_[offset + 1] =
        decodeRdsCharacter(static_cast<quint8>(blockD & 0xFF));

    psSegments_[static_cast<std::size_t>(segment)] = true;

    // Ein RDS-PS besteht aus vier Segmenten mit jeweils zwei Zeichen.
    // Bisher wurde psText_ schon nach JEDEM einzelnen Segment geändert.
    // Dadurch konnte z.B. aus "INFO" beim nächsten Scan vorübergehend
    // nur "IN" werden und der Scanner hat diesen Teiltext gespeichert.
    //
    // Jetzt wird ein neuer PS erst veröffentlicht, wenn alle vier
    // Segmente seit der letzten Veröffentlichung empfangen wurden.
    bool complete = true;

    for (bool seen : psSegments_) {
        if (!seen) {
            complete = false;
            break;
        }
    ...
    // Für die nächste PS-Runde erneut alle vier Segmente verlangen.
    // So kann auch ein dynamischer PS nicht aus alten und neuen
    // Segmenten zusammengemischt werden.
    psSegments_.fill(false);
}
...
void XdrClient::clearSignalData()
{
    ...
    const bool hadQuality = wam_ != -1 || usn_ != -1;
